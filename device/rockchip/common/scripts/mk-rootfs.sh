...
build_ubuntu()
{
	...
	message "=========================================="
	message "          Start building ubuntu 22.04"
	message "=========================================="

	if [ ! -f $RK_SDK_DIR/ubuntu/ubuntu.tar.gz.md5 ]; then
		if [ -d "$RK_SDK_DIR/ubuntu" ]; then
			rm -rf $RK_SDK_DIR/ubuntu
		fi
		mkdir -p $RK_SDK_DIR/ubuntu

		if [ "$RK_UBUNTU_GITHUB" = "y" ]; then
			git clone --progress https://github.com/luckfox-eng29/luckfox_lyra_ubuntu22.04.git $RK_SDK_DIR/ubuntu
		elif [ "$RK_UBUNTU_GITEE" = "y" ]; then
			git clone --progress https://gitee.com/luckfox-eng29/luckfox_lyra_ubuntu22.04.git  $RK_SDK_DIR/ubuntu
		fi
	fi

	if [ -f $RK_SDK_DIR/ubuntu/split_and_check_md5.sh ] && [ ! -f $RK_SDK_DIR/ubuntu/ubuntu.tar.gz ]; then
		cd $RK_SDK_DIR/ubuntu
		./split_and_check_md5.sh merge ubuntu.tar.gz
	fi

	if [ -f $RK_SDK_DIR/ubuntu/ubuntu.tar.gz ]; then
		"$RK_SCRIPTS_DIR/mk-ubuntu.sh" "$IMAGE_DIR"
	else
		error "Failed to retrieve Ubuntu rootfs. Please delete $RK_SDK_DIR/ubuntu and try again."
		exit
	fi
